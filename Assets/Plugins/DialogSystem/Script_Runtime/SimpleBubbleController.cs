...
{
    public class SimpleBubbleController : MonoBehaviour
    {
        ...
        public void StartPopping(string text, DialogSystem.DisplayDoneReceiver receiver)
        {
            this.str = text;
            pop = PopText(str);
            StartCoroutine(pop);
        ...
        IEnumerator PopText(string str)
        {
            for (int i = 1; i < str.Length; i++)
            {
                ...
                yield return new WaitForSeconds(DialogSystem.textDelay);
            }
            yield break;
        }
        ...
        void Update()
        {
            if (Input.GetKeyDown(DialogSystem.instance.inputSet.Confirm)) InteractHandler();
        }
